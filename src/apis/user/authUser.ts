...
export const register = async (registerRequest: RegisterRequest): Promise<void> => {
  return await SteakApi.post('/api/v1/user/auth/register', registerRequest)
}

export async function loginApiUserName(loginData: LoginRequest) {
  return await SteakApi.post('/api/v1/user/auth/username-login', loginData)
}

export async function loginApiEmail(loginData: LoginRequest) {
  return await SteakApi.post('/api/v1/user/auth/email-login', loginData)
}
