  ...
  <div class="text-white flex flex-col items-center justify-center px-4 min-h-screen">
    ...
    <form @submit.prevent="handleSubmitRegistry" class="w-full max-w-md max-auto space-y-4 mb-10">
      <div>
        ...
        <input v-model="account.email" type="email" name="email" id="email" placeholder="Nhập email của bạn"
      ...
      <div>
        ...
        <input type="text" v-model="account.username" placeholder="Tên đăng nhập"
          class="xl:w-full peer invalid:focus:border-red-500 h-12 px-4 rounded-lg bg-white dark:bg-[#1a1a1a] border border-black dark:border-gray-600 text-black dark:text-white invalid:focus:ring-2 invalid:focus:ring-red-500 focus:outline-none valid:focus:ring-2 valid:focus:ring-[#0af] transition w-full" />
      ...
      <div class="relative">
        ...
        <input :type="isPassword ? 'password' : 'text'" :placeholder="'Nhập mật khẩu của bạn'" name="password"
          id="password" v-model="account.password"
          class="xl:w-full peer invalid:focus:border-red-500 h-12 px-4 rounded-lg bg-white dark:bg-[#1a1a1a] border border-black dark:border-gray-600 text-black dark:text-white invalid:focus:ring-2 invalid:focus:ring-red-500 focus:outline-none valid:focus:ring-2 valid:focus:ring-[#0af] transition w-full" />
        <img @click="togglePasswordVisibility" :src="'https://ccdn.steak.io.vn/logo-on.svg'"
          class="absolute right-3 top-8 w-6 h-6 cursor-pointer" alt="Toggle Password" />
      ...
      <div>
        ...
        <input type="password" :placeholder="'Xác nhận mật khẩu của bạn'" name="verifyPassword" id="verifyPassword"
      ...
      <div>
        <label class="inline-flex items-start cursor-pointer relative">
          <input v-model="account.agreeNotification" type="checkbox" class="sr-only peer/notification" />
          <div
          ...
          <span class="ml-2 text-gray-500 dark:text-gray-400 font-bold">gửi cho tôi tin tức mới mất, khảo sát và ưu đãi
            đặc biệt từ steak store</span>
        </label>
      </div>

      <div>
        <label class="inline-flex items-start cursor-pointer relative">
          <input v-model="account.agreeTermsOfServices" type="checkbox" class="sr-only peer/terms shrink-0" />
