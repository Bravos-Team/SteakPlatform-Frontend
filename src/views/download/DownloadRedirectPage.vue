        ...
        <div
            ...
            <div
                ...
                <div class="flex flex-col md:flex-row gap-6">
                    <img src="https://ccdn.steak.io.vn/logo_steak.svg" alt="Game Cover"
                    ...
                    <div class="flex-1 text-white space-y-3">
                        ...
                        <p class="text-gray-300 text-sm text-wrap">
                            Steak là một nền tảng phân phối game đa nền tảng (cross-platform) hiện đại, cho phép game
                            thủ
                            tải xuống, cài đặt và cập nhật game một cách nhanh chóng, an toàn và tiện lợi.
                            Với giao diện tối giản nhưng mang đậm phong cách “gaming”, Steak mang đến trải nghiệm mượt
                            mà,
                            phù hợp cho cả game AAA lẫn indie.
                        </p>
                        <div v-if="releaseInfoLinux && isLinux" class="flex flex-wrap gap-4 text-sm text-gray-400">
                            <span>📦 <span class="text-white font-medium">{{
                                handleShowCapacity(releaseInfoLinux.size) }}</span></span>
                            <span>🛠️ Cập nhật: <span class="text-white font-medium">{{ new
                                Date(releaseInfoLinux.updated_at).toLocaleDateString('vi-VN', {
                                    dateStyle: 'full'
                                })
                                    }}</span></span>
                            <span>🌐 Ngôn ngữ: <span class="text-white font-medium">English</span>
                            </span>
                            <span class="flex items-center gap-x-1 text-center">
                                <HardDriveDownload class="size-4" />
                                <span class="text-center text-white font-medium flex ">Lượt
                                    tải:{{
                                        releaseInfoLinux.download_count }}</span>
                            </span>
                        </div>

                        <div v-if="releaseInfoWindows && isWindows" class="flex flex-wrap gap-4 text-sm text-gray-400">
                            <span>📦 <span class="text-white font-medium">{{
                                handleShowCapacity(releaseInfoWindows.size) }}</span></span>
                            <span>🛠️ Cập nhật: <span class="text-white font-medium">{{ new
                                Date(releaseInfoWindows.updated_at).toLocaleDateString('vi-VN', {
                                    dateStyle: 'full'
                                })
                                    }}</span></span>
                            <span>🌐 Ngôn ngữ: <span class="text-white font-medium">English</span>
                            ...
                            <span class="flex items-center gap-x-1 text-center">
                                ...
                                <span class="text-center text-white font-medium flex ">Lượt
                                    tải:{{
                                        releaseInfoWindows.download_count }}</span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 space-y-3">
                    ...
                    <div class="flex flex-col gap-y-2 gap-4">
                        <button v-if="isLinux" @click="handleDownloadDeb"
                        ...
                        <button v-if="isWindows" @click="handleDownloadWinSetup"
                            class="w-full font-mono tracking-widest ring-gray-300 ring-0 hover:ring-1  bg-white/10 hover:bg-white/20 transition-all duration-300 cursor-pointer rounded-sm py-1">
                ...
                <div v-if="isWindows && releaseInfoWindows"
                    class="mt-8 text-gray-300 text-sm bg-gray-900/70 p-4 rounded-xl border border-gray-700">
                    <p><span class="text-white font-medium">SHA-256:</span> {{ releaseInfoWindows.digest }}</p>
                    <!-- <p><span class="text-white font-medium">MD5:</span> 1bc29b36f623ba82aaf6724fd3b16718</p> -->
                </div>
...
const getInformations = async () => await axios.get('https://api.github.com/repos/Bravos-Team/SteakClient/releases/latest')
    .then(response => {
...
const handleDownloadDeb = () => {
    const raw = 'aHR0cHM6Ly9naXRodWIuY29tL0JyYXZvcy1UZWFtL1N0ZWFrQ2xpZW50L3JlbGVhc2VzL2Rvd25sb2FkL2xhc3Rlc3QvU3RlYWtfbGFzdGVzdC5kZWI='
    window.location.href = decodeURI(atob(raw));
};
const handleDownloadWinSetup = () => {
    const raw = 'aHR0cHM6Ly9naXRodWIuY29tL0JyYXZvcy1UZWFtL1N0ZWFrQ2xpZW50L3JlbGVhc2VzL2Rvd25sb2FkL2xhc3Rlc3QvU3RlYWtTZXR1cC5leGU='
    window.location.href = decodeURI(atob(raw));
}
