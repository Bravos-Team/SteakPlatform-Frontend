<template>
  ...
  <div v-else class="flex text-white flex-col h-full px-3">
    ...
    <div class="flex flex-row lg:w-full mb-[20px]">
      <div
        class="flex w-full justify-between flex-row items-center mx-[0px] me-[20px] ms-[0px] gap-x-[30px]"
      >
        ...
        <div class="flex gap-x-2 text-white/60">
          ...
          <span v-if="gameDetailData.data.latestVersionName === 'N/A'">
            {{ $t('newLatest') }}
          </span>
          <span v-else>
    ...
    <div class="grid grid-cols-12 gap-y-2 justify-between gap-x-4 w-full">
      <!-- LEFT CONTENT -->
      <div
        class="flex order-2 laptop:order-1 flex-col gap-y-[10px] relative col-span-12 laptop:col-span-9 w-full"
      >
        <div
        ...
        >
          ...
          <!-- RELATED GENRES & TAGS -->
          <div
            v-if="gameDetailData.data.genres.length && gameDetailData.data.tags"
            class="grid grid-cols-12 gap-x-6 gap-y-2"
          >
            <div
              v-if="gameDetailData.data.genres.length > 0"
              class="flex flex-col gap-y-2 col-span-12 desktop:col-span-6"
            >
              <span>Genres</span>
              <div
                class="flex gap-x-2 flex-wrap gap-y-2"
                v-if="gameDetailData.data?.genres?.length"
              >
                <span
                  ...
                  v-for="(genre, index) in gameDetailData.data?.genres"
                  :key="index"
                  >{{ genre.name }}</span
                >
              </div>
            ...
            <div
              v-if="gameDetailData.data.tags.length > 0"
              class="flex flex-col gap-y-2 col-span-12 desktop:col-span-6"
            >
              <span>Tags</span>
              <div class="flex flex-wrap gap-y-2 gap-x-2" v-if="gameDetailData.data?.tags?.length">
                <span
                  class="whitespace-nowrap cursor-pointer font-medium border border-white/20 text-white/80 text-sm px-3 py-1 rounded-[4px] bg-white/10 hover:bg-white/15 transition-colors duration-150"
                  v-for="(tag, index) in gameDetailData.data?.tags"
                  :key="index"
                  >{{ tag.name }}</span
                >
              </div>
        ...
        <div class="mt-[50px] mr-[0px] gap-y-[20px] mb-[0px] flex flex-col">
          ...
          >
            <div class="flex flex-col gap-[35px]">
              <div class="gap-[13px] flex flex-col text-pretty">
                <!-- CONTENT -->
                <div class="w-full flex flex-col gap-y-3 tablet:flex-row gap-x-[20px]">
                  <div class="flex flex-col gap-y-2 w-full">
                    <span class="text-[20px] w-full font-bold">Minimum</span>
                    <div class="flex flex-col">
                      <span class="w-auto text-[#ffffffa6] text-[15px]">OS version</span>
                      <span class="w-[11/12] text-pretty">{{
                  ...
                  <div class="flex w-full flex-col gap-y-2">
                    <span class="text-[20px] font-bold">Recommended</span>
                    <div class="flex flex-col">
                      <span class="w-auto text-[#ffffffa6] text-[15px]">OS version</span>
                      <span class="w-auto text-pretty">{{
