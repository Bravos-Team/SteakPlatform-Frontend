<template>
  <div class="text-black dark:text-white flex flex-col items-center justify-center px-4 min-h-screen">
    ...
    <form @submit.prevent="handleSubmission" novalidate class="w-full max-w-md mx-auto space-y-4">
      <div class="flex flex-col gap-y-1">
        ...
        <input type="text" required v-model="form.usernameOrEmail" autocomplete="username"
      ...
      <div>
        ...
        <div class="relative">
          <input :type="isPassword ? 'password' : 'text'" id="password" v-model="form.password"
            autocomplete="current-password" required :placeholder="$t('auth.informations.password_placeholder')"
          ...
          <img :src="'https://ccdn.steak.io.vn/logo-on.svg'" class="absolute right-3 top-3 w-6 h-6 cursor-pointer"
            alt="Toggle Password" @click="isPassword = !isPassword" />
        </div>
