    ...
    <Popover>
      ...
      >
        <div class="flex flex-col w-full">
          <div class="grid laptop:grid-cols-4 py-1 gap-x-20 bg-white/10 rounded-sm w-full">
            <span class="text-center text-lg font-black">Command</span>
            <span class="text-center text-lg font-black">Windows/Linux</span>
            <span class="text-end font-black text-l">macOS</span>

            <span class="text-center font-black text-l">Tips</span>
          </div>
          <scroll-area class="h-[20rem]">
            <div
              v-for="node in nodesTutorial"
              :key="node.id"
              class="flex justify-around gap-x-15 w-full border-b-1 mt-5"
            >
              <span class="text-sm text-center basis-64 flex items-center justify-center">{{
                node.title
              }}</span>
              <div
                v-if="node.inputs"
                class="grid laptop:grid-cols-2 gap-x-5 py-1 basis-200 last:border-b-0 pb-3"
              >
                <span
              ...
              <div
                v-else
                class="justify-center items-center grid grid-cols-2 basis-190 h-full pb-3"
              >
              ...
              <div class="flex gap-x-1 items-center basis-34 justify-center" v-if="node.short_key">
                <span
                  class="text-sm text-center flex items-center me-1 w-full"
              ...
              <div v-else class="flex justify-center items-center basis-34">
                <span class="w-full font-medium"> none </span>
              </div>
            </div>
          </scroll-area>
        </div>
