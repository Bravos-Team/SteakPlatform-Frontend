    ...
    >
      ...
      <div class="flex justify-center items-center w-full px-1 gap-3 laptop:gap-5">
        ...
        >
          ...
          />
          <input
            @keyup.enter="handleSearchingGames"
            v-model="searchValue"
            type="text"
      ...
      >
        <div class="flex gap-x-2 flex-wrap gap-y-2">
          <router-link
            :to="{ name: 'DiscoverGamesStore' }"
            class="bg-white/10 hover:bg-white/20 w-fit rounded-full px-4 py-1 text-white/60 hover:text-white font-medium"
          >
            {{ $t('discover') }}
          </router-link>
          <router-link
            :to="{ name: 'LeaderboardGamesStore' }"
            class="bg-white/10 hover:bg-white/20 rounded-full w-fit px-4 py-1 text-white/60 hover:text-white font-medium"
          >
            {{ $t('leaderBoardGames') }}
          </router-link>
        </div>
        <div class="flex flex-wrap gap-x-3">
          ...
          >
            <div
              ...
              class="absolute -top-1 -right-1 tablet:-top-3 tablet:-right-3 rounded-full bg-blue-500/50 group-hover:bg-blue-500 transition-colors duration-300 size-4 tablet:size-6 text-xs flex items-center justify-center"
            >
